...
export default class TextImagesAssessment extends Assessment {
	...
	calculateResultRegular(i18n) {
		...
		if (this.altProperties.withAltKeyword > 0) {
			return {
				...
				i18n.dgettext("js-text-analysis", "%1$sImage alt attributes%2$s: " + "Some images on this page contain alt attributes with words from your keyphrase! Good job!"), this._config.urlTitle, "</a>")
			};
		}
		...
		if (this.altProperties.withAltNonKeyword > 0) {
			return {
				...
				resultText: i18n.sprintf(
				/* Translators: %1$s and %2$s expand to links on yoast.com, %3$s expands to the anchor end tag */
				i18n.dgettext("js-text-analysis", "%1$sImage alt attributes%3$s: " + "Images on this page do not have alt attributes with words from your keyphrase. %2$sFix that%3$s!"), this._config.urlTitle, this._config.urlCallToAction, "</a>")
			};
		}
  ...
  */
	hasGoodNumberOfMatches() {
		return this.imageCount < 5 && this.altProperties.withAltKeyword > 0 || this.imageCount === 5 && inRangeStartEndInclusive(this.altProperties.withAltKeyword, 2, 4) || this.imageCount > 4 && inRangeStartEndInclusive(this.altProperties.withAltKeyword, this._minNumberOfKeywordMatches, this._maxNumberOfKeywordMatches);
	}
  ...
  */
   ...
   */
     ...
     * %5$s expands to the anchor end tag. */
				i18n.dgettext("js-text-analysis", "%3$sImage alt attributes%5$s: Out of %2$d images on this page, %1$d have alt attributes with " + "words from your keyphrase or synonyms. " + "That's a bit much. %4$sOnly include the keyphrase or its synonyms when it really fits the image%5$s."), this.altProperties.withAltKeyword, this.imageCount, this._config.urlTitle, this._config.urlCallToAction, "</a>")
			};
		}
