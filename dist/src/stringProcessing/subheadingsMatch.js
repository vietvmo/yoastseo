...
export default function (matches, keywordForms, locale) {
	...
	if (matches !== null) {
		...
		for (let i = 0; i < matches.length; i++) {
			...
			const formattedHeaders = replaceString(matches[i], removalWords);
			if (matchTextWithArray(formattedHeaders, keywordForms, locale).count > 0 || matchTextWithArray(matches[i], keywordForms, locale).count > 0) {
				foundInHeader++;
			}
		}
	}
